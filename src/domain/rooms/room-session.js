...
async function openSession({
	...
	return {
		...
		close: () => {
			...
			// Живой фидбек пользователя — раньше здесь стояло "финальный heartbeat
			// НЕ публикуется", и участник "призраком" висел у остальных до
			// истечения τ (45-60с) даже после явного ухода внутри приложения
			// (не после реального закрытия вкладки). Пересмотрено: "закрытие
			// вкладки — конец, без уборки" (ROOMS-SPEC §0) — про ЛОКАЛЬНУЮ
			// персистентность (IndexedDB, И7 ROOMS-MATH-v2.md — "гостевая
			// identity не персистится"), НЕ про сетевой протокол. Настоящий
			// абруптный обрыв (вкладка реально исчезла) физически не успевает
			// выполнить ни одной строки этой функции — τ-лизинг остаётся
			// ЕДИНСТВЕННЫМ путём для НЕГО, эта ветка для такого случая просто
			// никогда не запускается. А close(), вызванный ИЗ приложения (кнопка
			// закрытия/сворачивания оверлея), успевает отработать — best-effort
			// exit здесь избавляет ИМЕННО этот, самый частый, случай от
			// τ-задержки. ready — тот же гейт, что у heartbeat в tick(): нечего
			// объявлять ушедшим, если ни одного heartbeat ещё не было.
			if (ready) {
				transport.publish(buildRoomPresenceEvent(identity.privKey, kSess, hTopic, { type: "exit" }, now())).catch(() => {});
			}
