...
// MEDIA-PERF-TZ-5.md §2 — ВРЕМЕННО 1 (было 3), до задачи 3 (§4, "потоковая
// отдача 206"). Пока ответ SW не потоковый (`new Response(res.bytes)`),
// эта константа задаёт ДЛИНУ МОЛЧАНИЯ <video>, не размер буфера: живой
// замер §0 п.3 показал, что при 3с окно даёт ровно 3с тишины на каждый
// Range-ответ, и <video> сдаётся, переоткрывая Range с нуля — бесконечный
// цикл. Задача 3 возвращает это значение в 3 с обоснованием "теперь окно
// снова задаёт размер буфера, а не тишину".
export const WINDOW_TARGET_SECONDS = 1;
