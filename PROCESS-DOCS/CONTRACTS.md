...
### Стадии E/D/C — контракты пишутся отдельно, когда придёт их очередь
...
`contacts.jsx`, ни где-либо ещё) — на Этапе E для этого пункта либо
нечего делать (поля просто нет на экране), либо это отдельная,
незапланированная в ТЗ задача "показать `greeting` в списке входящих
заявок". Решение — за пользователем, вопрос будет задан явно при
старте Этапа E.
