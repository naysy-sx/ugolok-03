...
**Два отличия реализации от буквального текста REGLAMENT.md, оба —
временные/проектные, а не ошибки регламента:**

1. `.center` реализован как **`.measure`** — старый `.center` в
   `utilities` (max-width:70rem/`--container`+padding, для широкой
   колонки контента) ещё жив на время миграции (этап 66Б), одинаковое
   имя дало бы тихую коллизию в каскаде. Переименовать в `.center`
   вместе с удалением старого — этап 66В.
2. `.ratio` использует **`--aspect`**, не `--ratio` — в Уголке
...
рабочим") и полный список файлов — PLAN.md, "Этап 66". Не удалять
`.flow`/`.cluster`/`.grid-auto`/старый `.center` до тех пор, пока `grep`
по ним в `src/ui` не вернёт пусто.
