...
## Медиа-подсистема — живой фидбек №3: новый контур repeat + толщина

### `src/ui/icons/repeat.jsx` [C, переписан], `repeat-once.jsx` [новый файл]

Контур заменён на присланный пользователем (Tabler Icons "repeat"/
"repeat-once", MIT). `viewBox="0 0 24 24"` — НЕ `15 15`, как у
остальных иконок мини-бара; координаты не пересчитывались под общую
конвенцию (оригинальная точность контура важнее внутренней
viewBox-консистентности, невидимой пользователю). `repeat-once.jsx` —
та же геометрия + третий `<path d="M11 11l1 -1v4" />` (штрих "1").
`media-overlay.jsx` выбирает между ними условно: `session.repeat ===
"one" ? <IconRepeatOnce /> : <IconRepeat />`.

Раз цифра "1" теперь встроена в контур иконки, отдельный
`.media-mini-bar-repeat-badge` (кружок с цифрой поверх кнопки, этап 10)
стал избыточным дублированием — JSX и CSS правило удалены. **Решение
принято самостоятельно, не переспрошено отдельно у пользователя** —
обратимо: если понадобится вернуть badge вместе с новой иконкой,
достаточно восстановить оба места из git-истории.

### `src/styles/custom.css` [C, новый override]

`.media-mini-bar-btn.is-repeat .icon path { stroke-width: 2.24; }` —
новый контур использует дуги (SVG-команда `A`), не выражается через
`<polyline>/<line>`, как у соседних кнопок (nav-prev.jsx и т.п.),
значит идёт через `<path>` и ловит глобальный `.icon path{stroke-
width:0.6}` (строка ~122) — тоньше, чем у соседей. Специфичность
override (0,3,1) с запасом побеждает (0,1,1). Числовое значение —
пропорция от соседних 15-viewBox/1.4-stroke иконок: 1.4/15×24 ≈ 2.24.

### Живая проверка Chrome-автоматизацией

Все три состояния (`data-mode="off"/"all"/"one"`) подтверждены
скриншотами: приглушённый контур на off, заливной синий кружок с
чётко видимой иконкой на all, тот же кружок со встроенным штрихом "1"
на one — толщина линий визуально совпадает с play/restore рядом.
Пользователь параллельно проверял на своём реальном устройстве.

Регрессия 1976/1976, build стабильна.

## Медиа-подсистема — живой фидбек №4: корень проблемы с фоном кнопок мини-бара

### Диагностика (для будущих сессий — как искать подобное)

Симптом: `.media-mini-bar-btn.is-repeat[data-mode="off"]` имел
computed `background-color: var(--accent)`, ХОТЯ ни одно правило в
`custom.css`, найденное через `element.matches(selectorText)` по всем
`document.styleSheets`, этому свойству для этого элемента не
соответствовало (`matches` возвращал `false` для всех `[data-mode=
"all"/"one"]` правил). Инлайн-override (`style.setProperty` с
`important`) подтверждал: источник — реальный CSS-каскад, не
JS/анимация/inline-style. Изолированный тест (новый `<button>`,
`document.body.appendChild`, уникальные тестовые классы, НЕ связанные
с проектным CSS вообще) воспроизвёл ТОТ ЖЕ баг — значит причина не в
контексте/родителях/is-repeat вовсе, а в чём-то, матчащем ЛЮБУЮ
`<button>`. Нашлось в `minimal.css`: `button, [type="button"], ... {
background-color: var(--accent); ... }` внутри `@layer elements`.

**Ключевой урок про CSS Cascade Layers**: `custom.css` НЕ использует
`@layer` — эти правила unlayered, и по спецификации unlayered-стили
ВСЕГДА побеждают layered-стили НЕЗАВИСИМО от специфичности (это уже
задокументировано раньше в самом custom.css, строка ~4225, про
`.grow`). НО это правило "побеждает" действует ПО СВОЙСТВУ, не по
правилу целиком — если unlayered-код НЕ ЗАДАЁТ конкретное CSS-свойство
вовсе (как здесь: `.media-mini-bar-btn` никогда не трогал
`background`), каскаду для ЭТОГО свойства просто нечем "побеждать", и
единственный источник значения — layered `minimal.css` — применяется
как обычно. **Чинить minimal.css/@layer elements ЗАПРЕЩЕНО (`Известный
хвост`/базовый reset-слой проекта) — правильный путь всегда: явно
сбрасывать нужное свойство в unlayered custom.css.**

### `src/styles/custom.css` [C, критичный фикс]

`button[type="button"].media-mini-bar-btn` — добавлены `background:
transparent; border: 0;` (у `.media-overlay-btn` это стояло с этапа 1
изначально, `.media-mini-bar-btn`, транскрибированный из mockup'а на
этапе 8, — никогда). Итог бага: КАЖДАЯ кнопка мини-бара
(prev/next/close/restore/repeat-off) с этапа 8 незаметно была залита
акцентным фоном — визуально сливалась по тону с фоном плашки (оба
голубоватые/акцентные оттенки в обеих темах), из-за чего активная
repeat-заливка (та же заливка, намеренная) не выделялась на фоне
"тоже залитых" соседей.

Побочно найдено СРАЗУ ЖЕ после фикса: базовый `color: var(--accent-
contrast)` (живой фидбек №2) на честно прозрачном фоне дал ПОЧТИ
НЕВИДИМЫЕ иконки в тёмной теме — `--accent-contrast-dark: oklch(0.1 0
0)` (почти чёрный) спроектирован парой к ЯРКОМУ (0.74) акцентному
фону, а не к `--surface-raised-dark` (0.28) плашки. Заменено на
`var(--fg)` — theme-adaptive токен, корректный контраст с surface-
фоном в обеих темах без доп. логики. `:hover` `color` убран как
избыточный (совпадает с базой теперь).

### Живая проверка Chrome-автоматизацией

Тёмная тема (подтверждена `color-scheme: light dark` + фактические
oklch-значения на dark-варианте токенов): prev/play/next/restore/close
— белые (var(--fg)), чётко видны. repeat-off — приглушённая (var(--
muted)) версия. repeat-all/one — ЕДИНСТВЕННЫЕ (вместе с is-play)
заливные акцентные кружки, явно выделяются на общем фоне. Все три
состояния repeat визуально различимы между собой И от остального ряда
кнопок — что и было исходной жалобой пользователя ("у кнопки должно
быть два состояния", на деле все три были визуально неразличимы
