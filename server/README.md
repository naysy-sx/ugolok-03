...
- `api_addr`/`db_path` — `127.0.0.1:8080`, тот же принцип, что
  ...
  публичный интернет.
- `blobs_dir` — файлы блобов `{hh}/{hash}` рядом с sqlite (по умолчанию
  `dir(db_path)/blobs`). В SQLite остаются только метаданные.
...
`server/blossom/blossom-db/database.sqlite3` — метаданные блобов.
Байты — `server/blossom/blossom-db/blobs/{hh}/{hash}`. Старые базы, где
байты ещё в колонке `blob`: `server/blossom/migrate.sh` (идемпотентно;
`--drop-blob-column` только после полного прохода). GET без файла при
живой колонке делает dual-read и дописывает файл. Можно удалить каталог
`blossom-db/` для чистого старта (сервер должен быть остановлен).
