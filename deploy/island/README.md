...
**Чужие inbox-relay получателей.** Клиент больше не открывает WebSocket на relay из `kind:10050` получателя (иначе получатель, даже незнакомец, узнавал IP каждого, кто ему пишет). Событие уходит только на relay самого пользователя; между инстансами доставка — задача шлюза (GATEWAY-TZ-1). Мульти-инстансное развёртывание без шлюза может разрешить старое поведение флагом `"allowForeignInboxRelays": true` в `config.json` — тогда только `wss`, имена хостов (не IP-литералы и не частные сети), не более трёх.
