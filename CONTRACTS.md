...
6.7 (UI долей) — `[W]`, следующим шагом после 6.1-6.6. Ключ файла ВНУТРИ
расшаренного поддерева ("подключ" от `subtreeKey`) — контракт зафиксирован
в 6.6, но реализация читающей стороны (получатель расшифровывает файлы
доли БЕЗ обращения к `files_keys`, тот путь — только для СВОИХ файлов,
этап 53 И3 3.8) — отдельная, узкая правка `content.js`/`player-bridge.js`
по мере надобности (плеер/миниатюры ВНУТРИ смонтированной доли — не
запрошено явно, не входит в это урезание скоупа).
