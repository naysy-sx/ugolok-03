...
ТЗ на настройку VPS (`PROCESS-DOCS/VPS/TZ-ugolok-vps-grok-terminal.md`) явно запрещало ставить Node/CI-раннер на хост — предполагалось 2 ГБ RAM, swap заводили только под Docker+Caddy. Живая проверка (2026-09-06, внешний аудит безопасности) показала, что фактически на машине 4 ГБ — старое ограничение устарело, но сам принцип («раннер и сборка — гость на этом хосте, не единственный процесс») остаётся: Раннер (лейбл `ugolok`) и сборка в `node:22-bookworm` внутри Docker всё равно поставлены на этот хост — выбрано ради простоты (нет второй машины с сетевым доступом к `/var/www` и docker-сокету).
