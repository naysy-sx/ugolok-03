...
7. `docker system prune` раз в месяц — теперь есть таймер, `deploy/island/systemd/docker-prune.{service,timer}` (см. `deploy/island/README.md` "Обслуживание"), установить один раз.
8. `/root/.ssh/authorized_keys` при `PermitRootLogin no` безвреден, но можно убрать, чтобы не путать будущего себя.
9. Проверить 2FA на аккаунте `git.ugolok.tech` (Settings → Security) — для единственного аккаунта, чей push = root на сервере (§6.8), это обязательно, не опционально.

**Повторный прогон аудита (2026-09-06, после первого раунда фиксов) нашёл два новых важных факта**, оба меняют статус пунктов выше:

- **Токен раннера всё ещё НЕ перевыпущен** (`.runner` датирован 3 сентября — тот же скомпрометированный файл). Единственный срочный пункт, см. п.1.
- **`sudo systemctl reload caddy` на проде не подхватывает смену admin-адреса** (см. §6.9 живую проверку — прод-деплой конкретно на этом коммите упал ровно здесь, как и предполагалось) — значит на живом `ugolok.tech` до сих пор старый Caddy-процесс со старым admin API (`127.0.0.1:2019`) и БЕЗ HSTS на `ugolok.tech`/`git.ugolok.tech` (на `test.ugolok.tech` HSTS уже есть — там `apply-caddy.sh` работает в режиме `site`, глобальный Caddyfile не трогает, reload проходит штатно). **Один `sudo systemctl restart caddy` закрывает оба этих пункта разом** — файл на диске уже правильный, просто не применён.
- **Реальный `/opt/ugolok/island/coturn.conf` на VPS так и не переключён на `use-auth-secret`** (файл датирован до этой сессии, всё ещё `lt-cred-mech` со статическим паролем) — README (`deploy/island/README.md` "Секрет TURN") с самого этапа 6 описывает это как обязательный ручной шаг, который просто не был выполнен. Практическое следствие серьёзнее самой безопасности: `turncreds-server` уже выдаёт клиенту HMAC-креды (TURN REST API стиля), которые `lt-cred-mech`-coturn не понимает и отвергает — **TURN на проде сейчас, вероятно, не работает**, звонки между разными сетями (где реально нужен релей, не только STUN) могут падать. Требует ручной правки `coturn.conf` (взять `static-auth-secret` = тому же значению, что уже лежит в `TURN_STATIC_AUTH_SECRET` у `turncreds.env`) + добавить `denied-peer-ip`/квоты из `coturn.conf.example` + перезапустить контейнер coturn.

**Обновление (2026-09-07, живая проверка на VPS в рамках `artifacts/call-audit/09-FINAL-AUDIT.md`):** пункт выше закрыт. `/opt/ugolok/island/coturn.conf` сейчас использует `use-auth-secret`, секрет сверен по sha256 с `TURN_STATIC_AUTH_SECRET` контейнера `ugolok-turncreds-server` — совпадает. Контейнер `ugolok-coturn` работает с этим конфигом с 2026-09-06 08:29 UTC (момент последнего рестарта). Открытый вопрос — попадали ли жалобы пользователя на обрывы 2-10 минут на период ДО или ПОСЛЕ этого момента (см. `artifacts/call-audit/09-FINAL-AUDIT.md` §9, вопрос №1).

### 6.11 Трассировка звонков за флагом `?diag=1` (диагностика обрывов связи)

Реализация: `PROCESS-DOCS/VPS/AUDIO/TZ-diag-trace.md`, модуль
`src/core/diag/call-trace.js`. Формат записи — `docs/diag-trace-schema.md`.
Логику звонков не меняет; существующие ошибки (проглоченная ошибка публикации
в `call-runtime.js`, необработанный `ICE_FAILED` в `CONNECTED`, безусловный
`location.reload()` в `main.jsx`) **сознательно не исправлены** — задача
только даёт возможность их увидеть на реальном звонке.

**Как включить:**
- Добавить `?diag=1` к адресу приложения перед входом/до звонка (например
  `https://ugolok.tech/?diag=1`) — самый надёжный способ, действует с самого
  начала сеанса.
- Либо тумблер «Включить запись» на экране «Диагностика» — применится к
  СЛЕДУЮЩЕМУ звонку/входу в комнату, не к уже идущему (решение "писать или
  нет" принимается один раз при настройке звонкового рантайма).
- Пока запись идёт — красная плашка «Идёт запись» в левом нижнем углу, видна
  с любого экрана.

**Как снять запись:**
1. Дать звонку/сессии в «Быстрой связи» дойти до обрыва (или прогнать 10-15
   минут для контроля).
2. Экран «Диагностика» → панель «Трассировка звонков»:
   - **на настольном компьютере** — «Скачать файл» (`call-trace_<sid>_<UTC>.json`);
   - **на телефоне** — «Поделиться» (если доступен `navigator.share`) или
     «Скопировать в буфер» и вставить в сообщение/заметку — на части
     мобильных браузеров скачивание файла неудобно или недоступно.
3. Собрать снятые файлы С ОБЕИХ сторон звонка — сшивка по полю `t` (UTC),
   см. `docs/diag-trace-schema.md`.
4. Часы устройств не сверяются автоматически — если важна точность до
   секунды, сверить `env`-событие (нет прямого поля сдвига часов относительно
   NTP, только `utcOffsetMin` — смещение часового пояса, не дрейф).

**Экстренно, по кабелю:** `window.__ugolokTrace.rows()` / `.save()` /
`.clear()` в консоли — работает независимо от того, открыт ли экран
диагностики.

**Не забыть выключить** после диагностики — тумблер на экране «Диагностика»
(или просто закрыть вкладку с `?diag=1`, `sessionStorage` не переживает
закрытие вкладки). Запись не пишет содержимое сообщений/приватные ключи
(см. `docs/diag-trace-schema.md`, раздел «Санитизация»), но накопление за
часы работы вкладки создаёт файл, который незачем таскать за собой без нужды.
