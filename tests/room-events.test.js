...
test("PRESENCE heartbeat: build/parse round-trip -> готовый вход для presence.js's mergeHeartbeat", () => {
	...
	assert.deepEqual(parsed, { pubkey, type: "heartbeat", nick: "Гость", inVoice: false, at: AT_MS, receivedAt: AT_MS });
});
...
test("PRESENCE heartbeat: inVoice:true проходит round-trip (Этап 4)", () => {
	...
	const parsed = parseRoomPresenceEvent(event, kSess);
	assert.deepEqual(parsed, { pubkey, type: "heartbeat", nick: "Гость", inVoice: true, at: AT_MS, receivedAt: AT_MS });
});
...
test("PRESENCE exit: build/parse round-trip -> готовый вход для presence.js's mergeExit (без nick)", () => {
	...
	const parsed = parseRoomPresenceEvent(event, kSess);
	assert.deepEqual(parsed, { pubkey, type: "exit", at: AT_MS, receivedAt: AT_MS });
});
